...
User your favourite editor (vim, for instance):

.. prompt:: bash $

  vim /etc/ethereum/kintsugi/secrets.env
...
According to the Consensus Layer chosen (again, make sure that EL+CL) are in sync, let's 
enable 1 validator.

Lighthouse
~~~~~~~~~~

First, you need to write down the **Beacon Chain data directory**. It depends on the EL client 
previously launched. For instance, if you started :guilabel:`Geth` with :guilabel:`Lighthouse`, 
it will be ``/home/ethereum/.lh-geth/kintsugi/testnet-lh``
