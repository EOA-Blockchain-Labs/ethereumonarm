...
.. prompt:: bash $
  ...
  PRYSM_PASSWD (a ramdom password for the wallet)
  ETH1_FROM_ADDR (your Metamask address from Remy guide)
  ETH1_FROM_PRIV (your Metamask address private key from Remy guide)
...
We provided the necessary scripts and config files to make your own Ubuntu/Debian ARM64 packages so you 
can use build and use as your convenience.

The scripts will cross-compile (from x86_64 to ARM64) all 


Vagrant

In order to simplify the set up we recommend to install Vagrant and Virtualbox to use the Vagrantfile 
provided. It will provision and install all dependencies necessary to compile and package of the software.

Download our repository (Kintsugi branch)