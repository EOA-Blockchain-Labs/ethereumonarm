...
└────────────────────┬────────────────────┘
    ...
    ┌────────────────────────────────┐         ┌─────────────────┐
    │                                │         │                 │
    │     🔴 FORMAT DISK             │         │  🟢 PRESERVE    │
    │                                │         │     DATA        │
    │  1. Unmount all partitions     │         │                 │
    │  2. Wipe signatures (wipefs)   │         │  (ext4 without  │
    │  3. Create GPT table           │         │   format flag)  │
    │  4. Create partition           │         │                 │
    │  5. Format as ext4             │         └─────────────────┘
    │  6. Label: ethereum_data       │
    │                                │
    └────────────────────────────────┘
```
...
| Filesystem ≠ ext4 (LVM, RAID, LUKS, XFS, NTFS, etc.) | **FORMAT** |
| Filesystem = ext4 + `.format_me` flag exists | **FORMAT** |
| Filesystem = ext4 + no flag | **PRESERVE** ✓ |
